...
impl RequestResponseProtocol {
    ...
    async fn on_send_request(
        ...
        match self
            .handle
            .try_send_request(peer.into(), request, dial_options)
        {
            Ok(request_id) => {
                self.pending_inbound_responses.insert(
                    request_id,
                    PendingRequest::new(tx, Instant::now(), fallback_request),
                );
            }
            Err(error) => {
                log::warn!(
                ...
                self.metrics
                    .register_inbound_request_failure(error.to_string().as_ref());
            }
        }
    }
    ...
    fn on_inbound_response(
        ...
        match self.pending_inbound_responses.remove(&request_id) {
            ...
            Some(PendingRequest { tx, started, .. }) => {
                ...
                self.metrics
                    .register_outbound_request_success(started.elapsed());
            }
        }
    }
        ...
        match result {
            ...
            Ok(response) => {
                ...
                match sent_feedback {
                    ...
                    Some(feedback) => self
                        .handle
                        .send_response_with_feedback(request_id, response, feedback),
                }

                self.metrics
                    .register_inbound_request_success(started.elapsed());
            }
        }
    }
