...
impl Peerset {
    ...
    pub fn report_substream_opened(
        ...
        match state {
            ...
            state => {
                panic!(
                    "{}: invalid state for open substream {peer:?} {state:?}",
                    self.protocol
                );
            }
        }
    ...
    pub fn report_inbound_substream(&mut self, peer: PeerId) -> ValidationResult {
        ...
        match state {
            ...
            PeerState::Opening {
                direction: Direction::Outbound(reserved),
            } => {
                log::trace!(
                    target: LOG_TARGET,
        ...
        if is_reserved_peer {
            ...
            *state = PeerState::Opening {
                direction: Direction::Inbound(is_reserved_peer.into()),
            };
            return ValidationResult::Accept;
        }
        ...
        if let Poll::Ready(Some(action)) = Pin::new(&mut self.cmd_rx).poll_next(cx) {
            match action {
                ...
                PeersetCommand::SetReservedPeers { peers } => {
                    ...
                    let reserved_peers_maybe_remove = self
                        .reserved_peers
                        .difference(&peers)
                        .cloned()
                        .collect::<Vec<_>>();
