...
async fn inbound_substream_for_outbound_peer() {
    ...
    assert_eq!(
        peerset.report_inbound_substream(inbound_peer),
...
async fn set_reserved_peers() {
    ...
    to_peerset
        .unbounded_send(PeersetCommand::SetReservedPeers {
            peers: new_reserved_peers.clone(),
        })
        .unwrap();
...
async fn opening_peer_gets_canceled_and_disconnected() {
    ...
    to_peerset
        .unbounded_send(PeersetCommand::DisconnectPeer { peer })
        .unwrap();
...
async fn peer_disconnected_when_being_validated_then_rejected() {
    ...
    let peer = PeerId::random();
    assert_eq!(
        peerset.report_inbound_substream(peer),
        ValidationResult::Accept
    );
