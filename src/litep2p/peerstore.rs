...
impl PeerstoreHandle {
    ...
    fn new(
        ...
        Self(Arc::new(Mutex::new(PeerstoreHandleInner {
            peers,
            protocols,
            metrics,
        })))
...
impl PeerStoreProvider for PeerstoreHandle {
    fn is_banned(&self, peer: &PeerId) -> bool {
        self.0
            ...
            .peers
            .get(peer)
            .map_or(false, |info| info.is_banned())
    }
    ...
    fn report_peer(&self, peer_id: PeerId, change: ReputationChange) {
        ...
        lock.protocols
            .iter()
            .for_each(|handle| handle.disconnect_peer(peer_id.into()));
    ...
    fn peer_reputation(&self, peer: &PeerId) -> i32 {
        self.0
            ...
            .peers
            .get(peer)
            .map_or(0i32, |info| info.reputation)
    }
...
mod tests {
    ...
    fn report_banned_peers() {
        ...
        handle.report_peer(
            peer_a,
            sc_network_common::types::ReputationChange {
                value: i32::MIN,
                reason: "test".into(),
            },
        );
