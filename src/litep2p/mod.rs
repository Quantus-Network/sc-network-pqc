...
impl<B: BlockT + 'static, H: ExHashT> NetworkBackend<B, H> for Litep2pNetworkBackend {
        ...
        Self: Sized,
    {
        ...
        let executor = Arc::new(Litep2pExecutor {
        ...
        config_builder = notification_protocols
            ...
            .fold(config_builder, |config_builder, mut config| {
                config
                    .config
                    .set_handshake(Roles::from(&params.role).encode());
        ...
        let (mut request_response_receivers, request_response_senders): (
            ...
            .map(|config| {
                ...
                (
                    (config.protocol_name.clone(), rx),
                    (config.protocol_name.clone(), tx),
                )
            })
            .unzip();
        ...
        let known_addresses: HashMap<litep2p::PeerId, Vec<Multiaddr>> = known_addresses
            .into_iter()
            .fold(HashMap::new(), |mut acc, (peer, address)| {
                use sc_network_types::multiaddr::Protocol;
        ...
        let bandwidth: Arc<dyn BandwidthSink> = Arc::new(Litep2pBandwidthSink {
            sink: litep2p.bandwidth_sink(),
        });
