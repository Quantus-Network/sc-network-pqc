...
impl NetworkDHTProvider for Litep2pNetworkService {
    fn get_value(&self, key: &KademliaKey) {
        let _ = self
            .cmd_tx
            .unbounded_send(NetworkServiceCommand::GetValue { key: key.clone() });
    }

    fn put_value(&self, key: KademliaKey, value: Vec<u8>) {
        let _ = self
            .cmd_tx
            .unbounded_send(NetworkServiceCommand::PutValue { key, value });
    }
    ...
    fn put_record_to(
        ...
        let _ = self
            .cmd_tx
            .unbounded_send(NetworkServiceCommand::PutValueTo {
                record: Record {
    ...
    fn store_record(
        ...
        let _ = self
            .cmd_tx
            .unbounded_send(NetworkServiceCommand::StoreRecord {
                key,
                value,
                publisher,
                expires,
...
impl NetworkPeers for Litep2pNetworkService {
    fn set_authorized_peers(&self, peers: HashSet<PeerId>) {
        let _ = self
            .cmd_tx
            .unbounded_send(NetworkServiceCommand::SetReservedPeers {
    ...
    fn set_authorized_only(&self, reserved_only: bool) {
        let _ = self
            .cmd_tx
            .unbounded_send(NetworkServiceCommand::SetReservedOnly {
                protocol: self.block_announce_protocol.clone(),
                reserved_only,
            });
    }
    ...
    fn add_reserved_peer(&self, peer: MultiaddrWithPeerId) -> Result<(), String> {
        let _ = self
            .cmd_tx
            .unbounded_send(NetworkServiceCommand::AddPeersToReservedSet {
                protocol: self.block_announce_protocol.clone(),
    ...
    fn remove_reserved_peer(&self, peer: PeerId) {
        let _ = self
            .cmd_tx
            .unbounded_send(NetworkServiceCommand::RemoveReservedPeers {
                protocol: self.block_announce_protocol.clone(),
    ...
    fn sync_num_connected(&self) -> usize {
        self.peerset_handles
            ...
            .map_or(0usize, |handle| {
                handle.connected_peers.load(Ordering::Relaxed)
            })
    }
...
impl NetworkEventStream for Litep2pNetworkService {
    fn event_stream(&self, stream_name: &'static str) -> BoxStream<'static, Event> {
        ...
        let _ = self
            .cmd_tx
            .unbounded_send(NetworkServiceCommand::EventStream { tx });
        Box::pin(rx)
    }
}
