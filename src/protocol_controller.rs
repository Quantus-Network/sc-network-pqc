...
impl ProtocolHandle {
    ...
    pub fn add_reserved_peer(&self, peer_id: PeerId) {
        let _ = self
            .actions_tx
            .unbounded_send(Action::AddReservedPeer(peer_id));
    }
    ...
    pub fn remove_reserved_peer(&self, peer_id: PeerId) {
        let _ = self
            .actions_tx
            .unbounded_send(Action::RemoveReservedPeer(peer_id));
    }
    ...
    pub fn set_reserved_peers(&self, peer_ids: HashSet<PeerId>) {
        let _ = self
            .actions_tx
            .unbounded_send(Action::SetReservedPeers(peer_ids));
    }
    ...
    pub fn set_reserved_only(&self, reserved: bool) {
        let _ = self
            .actions_tx
            .unbounded_send(Action::SetReservedOnly(reserved));
    }
    ...
    pub fn disconnect_peer(&self, peer_id: PeerId) {
        let _ = self
            .actions_tx
            .unbounded_send(Action::DisconnectPeer(peer_id));
    }
    ...
    pub fn reserved_peers(&self, pending_response: oneshot::Sender<Vec<PeerId>>) {
        let _ = self
            .actions_tx
            .unbounded_send(Action::GetReservedPeers(pending_response));
    }
...
impl ProtocolHandleT for ProtocolHandle {
    fn disconnect_peer(&self, peer_id: sc_network_types::PeerId) {
        let _ = self
            .actions_tx
            .unbounded_send(Action::DisconnectPeer(peer_id.into()));
    }
}
...
impl ProtocolController {
    ...
    fn accept_connection(&mut self, peer_id: PeerId, incoming_index: IncomingIndex) {
        ...
        let _ = self
            .to_notifications
            .unbounded_send(Message::Accept(incoming_index));
    }
    ...
    fn reject_connection(&mut self, peer_id: PeerId, incoming_index: IncomingIndex) {
        ...
        let _ = self
            .to_notifications
            .unbounded_send(Message::Reject(incoming_index));
    }
    ...
    fn start_connection(&mut self, peer_id: PeerId) {
        ...
        let _ = self.to_notifications.unbounded_send(Message::Connect {
            set_id: self.set_id,
            peer_id,
        });
    }
    ...
    fn on_remove_reserved_peer(&mut self, peer_id: PeerId) {
        ...
        if let PeerState::Connected(direction) = state {
            ...
            if disconnect {
                ...
                assert!(
                    prev.is_none(),
                    "Corrupted state: reserved node was also non-reserved."
                );
            }
        } else {
            trace!(
    ...
    fn on_peer_dropped(&mut self, peer_id: PeerId) {
        self.on_peer_dropped_inner(peer_id)
...
mod tests {
    ...
    fn reserved_nodes_are_connected_dropped_and_accepted() {
        ...
        assert!(messages.contains(&Message::Connect {
            set_id: SetId::from(0),
            peer_id: reserved1
        }));
        assert!(messages.contains(&Message::Connect {
    ...
    fn both_reserved_nodes_and_nodes_supplied_by_peer_store_are_connected() {
        ...
        peer_store
            .expect_outgoing_candidates()
            .once()
            .return_const(outgoing_candidates);
    ...
    fn if_slots_are_freed_we_try_to_allocate_them_again() {
        ...
        peer_store
            .expect_outgoing_candidates()
            .once()
            .return_const(candidates1);
        peer_store
            .expect_outgoing_candidates()
            .once()
            .return_const(candidates2);
        peer_store
            .expect_report_disconnect()
            .times(2)
            .return_const(());
    ...
    fn enabling_reserved_only_mode_disconnects_regular_peers() {
        ...
        assert_eq!(messages.len(), 2);
        assert!(messages.contains(&Message::Drop {
            set_id: SetId::from(0),
            peer_id: regular1
        }));
        assert!(messages.contains(&Message::Drop {
    ...
    fn removed_connected_reserved_nodes_become_regular_in_non_reserved_mode() {
        ...
        assert!(matches!(
            controller.nodes.get(&peer1),
            Some(Direction::Inbound)
        ));
        assert!(matches!(
            controller.nodes.get(&peer2),
    ...
    fn incoming_request_for_connected_reserved_node_switches_it_to_inbound() {
        ...
        assert_eq!(
            ...
            Message::Accept(IncomingIndex(2))
        );
        assert_eq!(rx.try_recv().unwrap_err(), TryRecvError::Empty);
        assert!(matches!(
            controller.reserved_nodes.get(&reserved1),
        ...
        assert_eq!(
            rx.try_recv().ok().unwrap(),
            Message::Accept(IncomingIndex(3))
        );
        assert_eq!(rx.try_recv().unwrap_err(), TryRecvError::Empty);
        assert!(matches!(
            controller.reserved_nodes.get(&reserved2),
    ...
    fn incoming_request_for_connected_regular_node_switches_it_to_inbound() {
        ...
        assert_eq!(rx.try_recv().unwrap_err(), TryRecvError::Empty);
        assert!(matches!(
            controller.nodes.get(&regular1).unwrap(),
            Direction::Outbound,
        ));
        ...
        assert_eq!(
            rx.try_recv().ok().unwrap(),
            Message::Accept(IncomingIndex(0))
        );
        assert_eq!(rx.try_recv().unwrap_err(), TryRecvError::Empty);
        assert!(matches!(
            controller.nodes.get(&regular2).unwrap(),
            Direction::Inbound,
        ));
        ...
        assert_eq!(
            rx.try_recv().ok().unwrap(),
            Message::Accept(IncomingIndex(1))
        );
        assert_eq!(rx.try_recv().unwrap_err(), TryRecvError::Empty);
        assert!(matches!(
            controller.nodes.get(&regular1).unwrap(),
    ...
    fn incoming_request_for_connected_node_is_rejected_if_its_banned() {
        ...
        assert_eq!(
            rx.try_recv().ok().unwrap(),
            Message::Reject(IncomingIndex(1))
        );
        assert_eq!(rx.try_recv().unwrap_err(), TryRecvError::Empty);
        ...
        assert_eq!(
            rx.try_recv().ok().unwrap(),
            Message::Reject(IncomingIndex(2))
        );
        assert_eq!(rx.try_recv().unwrap_err(), TryRecvError::Empty);
