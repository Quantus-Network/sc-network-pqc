...
async fn validate_and_accept_substream() {
    ...
    let ValidationCallResult::WaitForValidation(result_rx) = handle
        .report_incoming_substream(peer_id, vec![1, 3, 3, 7])
        .unwrap()
    else {
...
async fn send_sync_notification() {
    ...
    assert_eq!(
        sync_rx.next().await,
        Some(NotificationsSinkMessage::Notification {
            message: vec![1, 3, 3, 8]
        })
    );
}
...
async fn send_async_notification() {
    ...
    notif
        .send_async_notification(&peer_id.into(), vec![1, 3, 3, 9])
        .await
        .unwrap();
    assert_eq!(
        async_rx.next().await,
...
async fn send_async_notification_to_non_existent_peer() {
    ...
    if let Err(error::Error::PeerDoesntExist(peer_id)) = notif
        .send_async_notification(&peer.into(), vec![1, 3, 3, 7])
        .await
    {
...
async fn backpressure_works() {
    ...
    assert!(
        futures::poll!(notif.send_async_notification(&peer_id.into(), vec![1, 3, 3, 9]))
            .is_pending()
    );
...
async fn peer_disconnects_then_async_notification_is_sent() {
    ...
    // as per documentation, error is not reported but the notification is silently dropped
    if let Err(error::Error::ConnectionClosed) = notif
        .send_async_notification(&peer_id.into(), vec![1, 3, 3, 7])
        .await
    {
    } else {
...
async fn cloned_service_opening_substream_works() {
    ...
    // validate inbound substream
    let ValidationCallResult::WaitForValidation(mut result_rx) = handle
        .report_incoming_substream(peer_id, vec![1, 3, 3, 7])
        .unwrap()
    else {
        panic!("peerset not enabled");
...
fn try_to_get_notifications_sink_for_non_existent_peer() {
    ...
    assert!(notif
        .message_sink(&sc_network_types::PeerId::random())
        .is_none());
}
...
async fn notification_sink_replaced() {
    ...
    handle
        .report_notification_sink_replaced(peer_id, new_sink)
        .unwrap();
