...
mod tests {
    ...
    fn protocol_info() {
        let upgrades = (1..=3)
            .map(|i| {
                ...
                upgrade.expect_protocol_info().return_once(move || {
                    vec![ProtoNameWithUsize(
                        ProtoName::from(format!("protocol{i}")),
                        i,
                    )]
                    .into_iter()
                });
        ...
        let protos = vec![
            ProtoNameWithUsize(
                ProtoNameWithUsize(ProtoName::from("protocol1".to_string()), 1),
                0,
            ),
            ProtoNameWithUsize(
                ProtoNameWithUsize(ProtoName::from("protocol2".to_string()), 2),
                1,
            ),
            ProtoNameWithUsize(
                ProtoNameWithUsize(ProtoName::from("protocol3".to_string()), 3),
                2,
            ),
        ];
